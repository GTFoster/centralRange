...
```{r}
missingBirds <- bmetaCent$node[is.na(bmetaCent$envarea)==T]
missingInsects <- imetaCent$node[is.na(imetaCent$envarea)==T]

for(i in 1:length(missingInsects)){
  starttime <- Sys.time()
  gbifQueryfilter(missingInsects[i])
  print(paste("Completed ", i, "out of ", length(missingInsects)))
